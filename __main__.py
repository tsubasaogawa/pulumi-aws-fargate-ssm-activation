...
activation_role = aws.iam.Role(
    f"{project_name}-activation-role",
...
activation_rpa = aws.iam.RolePolicyAttachment(
    f"{project_name}-activation-role-policy",
    role=activation_role.name,
    policy_arn="arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore",
)

activation = aws.ssm.Activation(
    f"{project_name}-activation",
    description=f"Activation for {project_name}",
    iam_role=activation_role.id,
    registration_limit=100,
    expiration_date=(datetime.datetime.now(tz) + expire_timedelta).strftime(
        "%Y-%m-%dT%H:%M:%SZ"
    ),
    opts=ResourceOptions(depends_on=[activation_rpa]),
)
...
task_rpa = aws.iam.RolePolicyAttachment(
    f"{project_name}-task-role-policy",
    role=task_role.name,
...
task_execution_rpa = aws.iam.RolePolicyAttachment(
